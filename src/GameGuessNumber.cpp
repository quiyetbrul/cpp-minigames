...
void GuessNumber::GameMechanics() {
  ...
  // who is playing?
  int player = inputInteger("Who is playing (1-Player, 2-Computer)?: ", 1, 2,"{GameGuessNumber.cpp}GuessNumber::GameMechanics()");
...
void GuessNumber::GamePlay() {
  ...
  do {
    GameMechanics();
    ans = inputChar("Play again? (y/n): ", 'y', 'n', "{GameGuessNumber.cpp}GuessNumber::GamePlay()");
    system("clear");
  } while (ans == 'y' || ans == 'Y');
}
